...
async function consolidateContacts(foundContact, email_, phoneNumber_) {
	try {
		...
		const secondaryContactIdArray = await findSecondaryContacts(
			foundContact
		);
...
async function bedrOneSecondary(
	...
	try {
		...
		if (
			secondaryPrecedentContact.linked_id !== primaryPrecedentContact.id
		) {
		...
		const primaryContact = await findPrimaryContact(
			secondaryPrecedentContact
		);
		const secondaryContactIdArray = await findSecondaryContacts(older);
